...
* and the socket descroptor											   *
***********************************************************************/
int Tcp::sendData(string data, int communication) {
    int data_len = data.length();
    const char * datas = data.c_str();
    int sent_bytes = send(this->isServer ? communication
                                         : this->socketDescriptor, datas, data_len, 0);

    // wait to get an acception that the messagae sent
    char buffer[3] = "ok";
    if (strcmp(datas, "ok") !=0){
        this->reciveData(buffer,2,communication);
    }

    if (sent_bytes < 0 || (strcmp(buffer,"ok") != 0) ) {
        //return an error represent error at this method
        return ERROR_SEND;
    }
    //return correct if there were no problem
...
***********************************************************************/
int Tcp::reciveData(char* buffer, int size, int communication) {
    int read_bytes = recv(this->isServer ? communication
                                         : this->socketDescriptor, buffer, size, 0);

    // send "no" if the message didnt send
    if(read_bytes <= 0){
        this->sendData("no", communication);
    // send "ok" if the message sent
    } else if(strcmp(buffer, "ok") !=0){
        this->sendData("ok",communication);
    }
